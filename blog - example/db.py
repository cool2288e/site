...
def init_db():
    with get_db() as db:
        ...
        # ДОДАЛИ КОЛОНКУ title (заголовок)
        db.execute("""
...
def get_section_posts(section_id):
    with get_db() as db:
        # ORDER BY id ASC щоб виводити в тому порядку, як ми додали (Теорія 1, потім 2 і т.д.)
        return db.execute("SELECT * FROM posts WHERE section_id = ? ORDER BY id ASC", (section_id,)).fetchall()
